    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <Badge variant={kelayakan.data ? "default" : "destructive"}>
          {kelayakan.data === undefined
            ? "Memeriksa…"
            : kelayakan.data
              ? "Layak meminjam"
              : "Diblokir (denda / terlambat)"}
        </Badge>
      </div>
      ...
      <Tabs defaultValue="katalog">
        ...
        <TabsContent value="katalog" className="mt-4">
          <Katalog dapatPinjam={!!kelayakan.data} />
        </TabsContent>
                ...
                <div className="flex gap-2 pt-1">
                  {bisaPinjam ? (
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      disabled={!dapatPinjam}
                      onClick={async () => {
                        try {
                          await reservasi({ data: { buku_id: b.id } });
                          toast.success("Reservasi tercatat.");
                          qc.invalidateQueries({ queryKey: ["reservasi-saya"] });
                        } catch (e) {
...
function PinjamanSaya() {
  ...
  const aktif =
    pinjaman.data?.filter((p: any) =>
      ["menunggu", "disetujui", "dipinjam", "terlambat"].includes(p.status),
    ) ?? [];
  const riwayat =
    pinjaman.data?.filter((p: any) => ["dikembalikan", "ditolak"].includes(p.status)) ?? [];

  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Peminjaman aktif ({aktif.length})</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Reservasi ({reservasi.data?.length ?? 0})</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>Buku</TableHead>
                <TableHead>Status</TableHead>
                <TableHead>Batas ambil</TableHead>
                <TableHead />
              </TableRow>
            </TableHeader>
            <TableBody>
              {reservasi.data?.map((r: any) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell>
                    ...
                    >
                      {r.status}
                      {r.status === "menunggu" ? ` (#${r.posisi_antrian})` : ""}
                    </Badge>
                  </TableCell>
            ...
            <TableBody>
              {denda.data?.map((d: any) => (
                <TableRow key={d.id}>
                  ...
                  <TableCell>
                    <Badge variant={d.status === "belum_bayar" ? "destructive" : "default"}>
                      {d.status}
                    </Badge>
                  </TableCell>
                </TableRow>
              ...
              {riwayat.map((p: any) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell className="text-xs">
                    {fmtWITA(p.tanggal_kembali ?? p.tanggal_pengajuan)}
                  </TableCell>
                </TableRow>
              ))}
              {!riwayat.length && (
